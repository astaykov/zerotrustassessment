...
.NOTES
    ...
    Required permission: ThreatSubmission.Read (delegated) or ThreatSubmission.Read.All (application)
#>
...
function Test-Assessment-41117 {
    ...
    if ($queryError) {
        $message = if ($httpStatus -in (401, 403)) {
            "⚠️ Microsoft Graph returned HTTP $httpStatus while reading email threat submissions. Verify that delegated **ThreatSubmission.Read** or application **ThreatSubmission.Read.All** permission is consented and that the assessment identity has the Security Reader role, then re-run the assessment."
        }
        elseif ($httpStatus -eq 404) {
            '⚠️ The Microsoft Graph beta email threat submissions endpoint returned HTTP 404 in a supported Global tenant. Verify API provisioning and availability, then re-run the assessment.'
        }
        else {
